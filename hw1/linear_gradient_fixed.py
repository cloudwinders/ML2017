...
theta = np.zeros(shape=((train_x.shape[1]), 1))

theta, J_history_train, J_history_val = gradient_descent(train_x, train_y, theta, alpha, iterations, ratio)
...
print("train last cost: \t\t" + str(J_history_train[J_history_train.shape[0]-1]))
print("validation last cost: \t\t" + str(J_history_val[J_history_val.shape[0]-1]))
...
axarr[0].plot(np.arange(iterations), J_history_train)
axarr[0].set_title('Cost Function: Train data vs Validation data')
axarr[1].scatter(np.arange(iterations), J_history_val)
f.text(0.5, 0.04, 'Iterations', ha='center')
f.text(0.04, 0.5, 'Cost Function', va='center', rotation='vertical')
'''

first_legend = 'train ' + str(ratio*100) + '%'
second_legend = 'validation ' + str((1-ratio)*100) + '%'
plt.title('Prediction of PM2.5')
plt.xlabel('Iterations')
plt.ylabel('error')
plt.plot(np.arange(iterations), J_history_train, "r-o", lw=1)
plt.plot(np.arange(iterations), J_history_val, "b:", lw=1)
plt.legend([first_legend , second_legend], loc='upper left')
plt.axis([0, iterations, 5.5, 8])
plt.show()

#.........................................Save File.........................................

np.savetxt("./result/theta.csv", theta, delimiter = ",", fmt = "%s")
#np.savetxt("./result/J_history_train.csv", J_history_train, delimiter = ",", fmt = "%s")
#np.savetxt("./result/J_history_test.csv", J_history_val, delimiter = ",", fmt = "%s")
